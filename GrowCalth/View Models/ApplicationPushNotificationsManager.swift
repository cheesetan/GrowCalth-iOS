...
class ApplicationPushNotificationsManager: ObservableObject {
    ...
    func updateFCMTokenInFirebase(fcmToken: String) async throws {
        do {
            try await Firestore.firestore()
                .collection("schools")
                .document("sst").collection("fcmTokens").document(UIDevice.current.identifierForVendor?.uuidString ?? "").setData([
                    "token": fcmToken
                ])
        } catch {
    ...
    nonisolated internal func fetchFCMTokensFromFirebase() async throws -> [String] {
        do {
            let query = try await Firestore.firestore()
                .collection("schools")
                .document("sst").collection("fcmTokens").getDocuments()
    ...
    nonisolated internal func fetchSpecifiedFCMTokenDocumentIdsRequest(
        ...
        do {
            let query = try await Firestore.firestore()
                .collection("schools")
                .document("sst").collection("fcmTokens").whereField("token", isEqualTo: fcmToken).getDocuments()
    ...
    internal func removeSpecifiedFCMTokenRequest(documentID: String) async throws {
        do {
            try await Firestore.firestore()
                .collection("schools")
                .document("sst").collection("fcmTokens").document(documentID).delete()
        } catch {
    ...
    nonisolated internal func fetchPrivateKeyForOAuth() async throws -> String {
        do {
            let document = try await Firestore.firestore()
                .collection("settings")
                .document("private-key-for-oauth")
